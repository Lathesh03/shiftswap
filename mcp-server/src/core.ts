// Copied from ../../lib/scheduling/core.ts — the framework-agnostic tool
// logic, unchanged except for relative import paths. See the project
// README's note on why this is duplicated into the MCP server rather than
// imported cross-project (different module systems, learning-build tradeoff).
import type { SupabaseClient } from "@supabase/supabase-js";
import { assertTransition, deriveState, rowToEvent } from "./swaps.js";
...
export async function proposeMatch(supabase: SupabaseClient, swapId: string, candidateId: string) {
  // 1. Re-validate the rules server-side. NEVER trust that the caller checked.
  const rules = await checkLaborRules(supabase, candidateId, swapId);
  if (!rules.allowed) {
    return { ok: false, error: `Match rejected: ${rules.reasons.join(" ")}` };
  }

  // 2. Guard the state transition (same guard the web app's buttons use).
  const { data: rows } = await supabase
    .from("swap_events").select("*").eq("swap_id", swapId)
    .order("created_at", { ascending: true });
